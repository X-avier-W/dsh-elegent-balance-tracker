 ...
 *   per billing bucket in the form "空闲时段 A元 B元 C元 高峰时段 D元 E元 F元"
 *   (three columns = three models, in the same order as the model list), for
 *   input-cache-hit, input-cache-miss, then output.
...
function parseOfficialPricing(html) {
  ...
  // Parse the three billing buckets sequentially: each bucket is
  // "百万tokens<kind> 空闲时段 a元 b元 c元 高峰时段 d元 e元 f元".
  const buckets = [];
  const rowRe = /百万tokens(输入\s*[（(]缓存命中[）)]|输入\s*[（(]缓存未命中[）)]|输出)\s*空闲时段\s*([0-9.]+)元\s*([0-9.]+)元\s*([0-9.]+)元\s*高峰时段\s*([0-9.]+)元\s*([0-9.]+)元\s*([0-9.]+)元/g;
  let m;
  while ((m = rowRe.exec(text)) !== null) {
    buckets.push({
      kind: m[1].includes("缓存命中") ? "cacheRead" : m[1].includes("未命中") ? "input" : "output",
      idle: [Number(m[2]), Number(m[3]), Number(m[4])],
      peak: [Number(m[5]), Number(m[6]), Number(m[7])]
    });
  }
  ...
  const cols = Math.min(modelIds.length, 3);
  const models = {};
  for (let c = 0; c < cols; c++) {
    const id = modelIds[c];
    const read = (kind) => {
      const cell = byKind[kind];
      return cell === void 0 ? void 0 : { idle: cell.idle[c], peak: cell.peak[c] };
    };
